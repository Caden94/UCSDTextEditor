...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for (int i=0; i<word.length(); i++) {
			if (i == word.length()-1 && numSyllables>0 && Character.toLowerCase(word.charAt(i))=='e' && newSyllable) {
				numSyllables--;
			}
			if (newSyllable && vowels.indexOf(Character.toLowerCase(word.charAt(i)))>=0) {
				newSyllable = false;
				numSyllables++;
			} 
			else if (vowels.indexOf(Character.toLowerCase(word.charAt(i)))<0) {
				newSyllable = true;
			}
		}
